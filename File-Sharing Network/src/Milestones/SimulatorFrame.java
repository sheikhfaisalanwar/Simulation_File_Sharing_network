 ...
 * components)
 * 
 * @author Sheikh Faisal Anwar, Vanja Veselinovic, Nicholas Robidoux, Muhammad
 *         Kashif Siddiqui
 *
 */
public class SimulatorFrame extends JFrame implements ActionListener, Serializable {
	...
	private JMenuItem clearText, save, load, undo;
	...
	protected JList<Document> topkdocs;
	...
	protected DefaultListModel<Document> topKResults = new DefaultListModel<Document>();
	...
	public SimulatorFrame() {
		...
		//Setting up Top K Documents pane
		topkdocs = new JList<Document>(topKResults);
		topk.add(new JLabel("Top Document(s)"), BorderLayout.NORTH);
		topk.add(topkdocs, BorderLayout.CENTER);
		topkdocs.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		topk.setBorder(new EmptyBorder(5, 5, 5, 5));
	...
	/**
	 * Method to get the string representation of  the text area
	 * 
	 * @return String value
	 */
	...
	/**
	 * method for setting undo button to enabled from outside SimulatorFrame,
	 * specifically from Simulator after the first simulation cycle
	 */
	public void undoSetEnabled(){
		undo.setEnabled(true);
	}
	
	/**
	 * Method to return the count of how many simulation cycles have occurred
	 * @return int countCycles
	 */
	public int getCountCycles(){
		return countCycles;
	}
	
	public void iterateCountCycles(){
		countCycles++;
	}
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (event.getSource() == buttonInitializeSim) {
			...
			while (simulations > 100){
				JOptionPane.showMessageDialog(null, "THAT'S TOO BIG", "Error", JOptionPane.ERROR_MESSAGE);
				text2 = JOptionPane.showInputDialog("Enter number of simulation iterations desired: ");
				simulations = checkInt(text2);
			}
		...
		// View selected User's liked Documents
			...
			else if (selected.getLiked().size() == 0) {
		...
		// what happens when load is clicked
			...
			if(f.exists() && !f.isDirectory()) { //check if file exists
				...
				JOptionPane.showMessageDialog(null, "File does not exist! ", null, JOptionPane.ERROR_MESSAGE);
			}
		}
		
		else if(event.getSource() == undo){
			String name = "autoSaves/autoSave"+(countCycles - 1)+".txt";
			SimulatorFrame frame = this.readObject(name);
			this.dispose();
			frame.pack();
			frame.setVisible(true);
		}
	...
	public void autoSave(){
		writeObject("autoSaves/autoSave"+(countCycles)+".txt");
	}
